...
const RequestGroupsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3001/arrangements/manager/",
        {
          method: "GET",
          headers: {
  ...
  const handleApprove = async (groupId) => {
    try {
      const url = `http://localhost:3001/arrangements/manager/approve/${groupId}`;
  ...
  const handleReject = async () => {
    try {
      const url = `http://localhost:3001/arrangements/manager/reject/${selectedGroupId}`;
      const response = await fetch(url, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleUndo = async (groupId) => {
    try {
      const url = `http://localhost:3001/arrangements/manager/undo/${groupId}`;
      const response = await fetch(url, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
