    ...
    const fetchSchedules = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `{process.env.API_URL}/schedules/manager/?start_date=${formattedDate}&end_date=${formattedDate}`,
          {
            headers: {
          ...
          return (
            <Card key={role} style={{ marginBottom: "15px" }}>
              <CardContent>
                ...
                {roleSchedule["Work from home"] && roleSchedule["Work from home"].length > 0 && (
                  <div><br />
                  <Typography variant="h7" style={{ textDecoration: "underline" }}>
                    Work from home:<br />
                  </Typography>
                ...
                {roleSchedule["Work from home (AM)"] && roleSchedule["Work from home (AM)"].length > 0 && (
                  <div><br />
                  <Typography variant="h7" style={{ textDecoration: "underline" }}>
                    Work from home (AM):<br />
                  </Typography>
                  <Typography variant="body2">
                ...
                {roleSchedule["Work from home (PM)"] && roleSchedule["Work from home (PM)"].length > 0 && (
                  <div><br />
                  <Typography variant="h7" style={{ textDecoration: "underline" }}>
                    Work from home (PM):<br />
                  </Typography>
                  <Typography variant="body2">
    ...
    return (
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} size="small">
          <TableHead>
            <TableRow>
              <TableCell sx={{ width: "16.66%", fontWeight: "bold" }}>Role</TableCell> {/* 2/12 grid */}
              <TableCell sx={{ width: "41.67%", fontWeight: "bold" }}>In Office</TableCell> {/* 5/12 grid */}
              <TableCell sx={{ width: "41.67%", fontWeight: "bold" }}>Work from Home</TableCell> {/* 5/12 grid */}
            </TableRow>
          </TableHead>
          <TableBody>
            {Object.keys(filteredSchedules).map((role) => {
              ...
              return (
                <TableRow key={role}>
                  <TableCell sx={{ width: "16.66%" }}>{role}</TableCell> {/* 2/12 grid */}
                  <TableCell sx={{ width: "41.67%" }}>{(roleSchedule["In office"] || []).join(", ")}</TableCell> {/* 5/12 grid */}
                  <TableCell sx={{ width: "41.67%" }}>{renderWFHCell()}</TableCell> {/* 5/12 grid */}
                </TableRow>
              );
            })}
          </TableBody>
